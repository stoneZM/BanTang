...
-(void)archiveObjc:(id)obj setupAllowedPropertyNames:(NSArray*)names identifier:(NSString *)identifier{
    ...
    NSString* filePath = [NSString stringWithFormat:@"%@/%@+%@.data",path,objName,identifier];
    [NSKeyedArchiver archiveRootObject:obj toFile:filePath];
}

//实现解档  className:要解档的类名
-(void)decodeObjForClassName:(NSString *)className
    ...
    NSString* filePath = [NSString stringWithFormat:@"%@/%@+%@.data",archPath,className,identifier];
